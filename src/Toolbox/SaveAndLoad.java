...
public class SaveAndLoad {
    ...
    public static void load(Map map, String name, Settings settings)
    {
        map.getBallList().clear();
        map.getObstacleList().clear();
        map.getSurfaceList().clear();
        ...
        try {
            while (true) {
                ...
                if (line.startsWith("Ball")&&countBall<settings.getnHuman()+settings.getnBot()) {
                    if (ballModel == null) {
                        ballModel = OBJLoader.loadObjModel("ball", map.getLoader());
                    }
                    ModelTexture white = new ModelTexture(map.getLoader().loadTexture("white"));
                    TexturedModel model = new TexturedModel(ballModel, white);
                    //System.out.println(currentLine[6]+" "+ currentLine[7]+" "+currentLine[8]+" "+currentLine[1]+" "+ currentLine[2]+" "+ currentLine[3]+" "+ currentLine[4]);
                    map.getBallList().add(new Ball(new Entity(model,new Vector3f(Float.parseFloat(currentLine[5]),
                            Float.parseFloat(currentLine[6]), Float.parseFloat(currentLine[7])),
                            Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]),
                            Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4]) )));
                    countBall++;
                    ...
                    map.getPutHole().setPosition(new Vector3f(Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3])));

                } else if (line.startsWith("Terrain")) {
                    map.getTerrain().changeSize(Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4]), map.getLoader());


                } else if (line.startsWith("cube")) {
                    if(cubeModel==null) {
                        cubeModel = OBJLoader.loadObjModel("cube", map.getLoader());
                    }
                    ModelTexture box = new ModelTexture(map.getLoader().loadTexture("box"));
                    TexturedModel model = new TexturedModel(cubeModel,box);
                    map.getObstacleList().add(new Obstacle(new Entity(model,new Vector3f(Float.parseFloat(currentLine[5]), Float.parseFloat(currentLine[6]), Float.parseFloat(currentLine[7])), Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4])), "cube"));

                } else if (line.startsWith("slope")) {
                    if(slopeModel==null) {
                        slopeModel = OBJLoader.loadObjModel("slope", map.getLoader());
                    }
                    ModelTexture wood = new ModelTexture(map.getLoader().loadTexture("wood"));
                    TexturedModel model = new TexturedModel(slopeModel,wood);
                    map.getObstacleList().add(new Obstacle(new Entity(model,new Vector3f(Float.parseFloat(currentLine[5]), Float.parseFloat(currentLine[6]), Float.parseFloat(currentLine[7])), Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4])), "slope"));


                } else if (line.startsWith("bar")) {
                    if(barModel==null) {
                        barModel = OBJLoader.loadObjModel("bar", map.getLoader());
                    }
                    ModelTexture bar = new ModelTexture(map.getLoader().loadTexture("bar"));
                    TexturedModel model = new TexturedModel(barModel,bar);
                    map.getObstacleList().add(new Obstacle(new Entity(model,new Vector3f(Float.parseFloat(currentLine[5]), Float.parseFloat(currentLine[6]), Float.parseFloat(currentLine[7])), Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4])), "bar"));
                    ...
                    RawModel mudel = map.getLoader().loadToVAO(vertexArray, textureArray, normalArray, indices);
                    Vector3f[] points= new Vector3f[3];
                    for(int i=0;i<3;i++){
                        points[i] = new Vector3f(vertexArray[i*3],vertexArray[i*3+1],vertexArray[i*3+2]);
                    }
                    ...
                    if(line.startsWith("mud")){
                        actualName = "mud";
                        if(mud==null){
                            mud = new ModelTexture(map.getLoader().loadTexture(actualName));
                        }
                        model = new TexturedModel(mudel,mud);
                    }
                    else if(line.startsWith("sand")){
                        actualName = "sand";
                        if(sand==null){
                            sand = new ModelTexture(map.getLoader().loadTexture(actualName));
                        }
                        model = new TexturedModel(mudel,sand);
                    }
                    ...
                    map.getSurfaceList().add(new Surface(new Entity(model, new Vector3f(Float.parseFloat(currentLine[6]), Float.parseFloat(currentLine[7]), Float.parseFloat(currentLine[8]))
                            ,Float.parseFloat(currentLine[1]),Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4])),Float.parseFloat(currentLine[5]), points, actualName));
