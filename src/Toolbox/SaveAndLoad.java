...
public class SaveAndLoad {
    ...
    private File file = new File("C:/Users/Henri/IdeaProjects/Project2/save.txt");
    private static RawModel ballModel, cubeModel, barModel, slopeModel;

    public static void save(Terrain terrain, List<Obstacle> obstacles, List<Ball> balls, List<Surface> surfaces, PutHole putHole, RawModel model) throws FileNotFoundException {
        ...
        for(Surface surf: surfaces)
        {
            ...
            for(int i = 0; i<9; i++) {
                writer.print(" " + model.getVerticesArray()[i]);
            }
            writer.println();
        }
    ...
    public static void load(Terrain terrain, List<Obstacle> obstacles, List<Ball> balls, List<Surface> surfaces, PutHole putHole, Loader loader, List<ModelTexture> textures)
    {
        FileReader fr = null;
        String line;
        ...
        try {
            while (true) {
                ...
                if (line.startsWith("Ball")) {
                    ...
                    balls.add(new Ball(new Entity(model,new Vector3f(Float.parseFloat(currentLine[6]), Float.parseFloat(currentLine[7]), Float.parseFloat(currentLine[8])), Float.parseFloat(currentLine[1]), Float.parseFloat(currentLine[2]), Float.parseFloat(currentLine[3]), Float.parseFloat(currentLine[4]) )));
                    ...
                    if(line.startsWith("mud")){
                        actualName = "mud";
                        model = new TexturedModel(mudel, new ModelTexture(loader.loadTexture("mud")));}
                    else if(line.startsWith("sand")){
                        actualName = "sand";
                        model = new TexturedModel(mudel, new ModelTexture(loader.loadTexture("sand")));}
